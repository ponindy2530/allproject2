...
@Component({
    ...
    templateUrl: 'calculator.component.html',
     styleUrls: ['javascript-calculator.css'],
    host: {
...
export class CalculatorComponent {
    ...
    expression: string = '=';
    percentPattern = new RegExp('[0-9]+[+-]{1}[0-9]+(%)$');
  percentPattern2 = new RegExp('^(%){1}$');
    constructor() { }
...
clearLcd(){
    ...
    //    console.log('test'+this.percentPattern.test(this.lcdValue));
        
    if(this.percentPattern.test(this.lcdValue)){
//let ar = this.lcdValue.split(/(+-%)/);
let ar = this.lcdValue.split('+').join(',').split('-').join(',').split('%')
console.log(ar); let tt = ar[0].split(',');
 let op:string;
if(this.lcdValue.indexOf('+') !== -1){
   op ="+";
}else{
    op="-";
}
let pc = Number(tt[1])/100*Number(tt[0]);
let result = tt[0] + op + pc;// tt[1];
console.log('result ='+result);
let rr:number = eval(result);
console.log('rr='+rr);
 this.expression =this.lcdValue;
this.lcdValue = rr.toString();
return;
        }
    ...
    convertToPolishNotation(input:any) {
        let output: string = "";
    ...
    solvePolishNotation(polish:any) {
